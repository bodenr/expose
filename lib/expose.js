...
function defaultOpts(opts, parentId) {
    ...
    return mixin({targets: targets,
            grep: [/\.js$/],
            ungrep: opts.ungrep && opts.ungrep.lenght
                    ? opts.ungrep
...
var NM_SEG = '(?:' + path.sep + '?node_modules' + path.sep + '|' + path.sep + '?node_modules$){1}';

/**
 * Build defaul ungrep pattern based on targets.
 *
 *
 * @param {Array} targets
 * @api private
...
function defaultUngrep(targets) {
    if (targets && targets.length) {
        ...
        targets.forEach(function(target) {
            target = '(?:^' + path.dirname(target) + path.sep + '){1}';
            target += '(?:/?.*/?)*' + NM_SEG;
            ungreps.push(new RegExp(target));
        });
        return ungreps;
    }
    return new RegExp(NM_SEG);
};
